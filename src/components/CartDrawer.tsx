      ...
      {isCartOpen && (
        <>
          ...
          >
            ...
            {/* Header */}
            <div className="flex items-center justify-between p-3 sm:p-6 border-b border-border/50 bg-background/40 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-2xl bg-primary/10 flex items-center justify-center text-primary">
                  <svg className="w-4 h-4 font-bold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                  </svg>
                </div>
                <div>
                  <h2 className="font-display text-base sm:text-xl font-bold text-foreground">My Cart</h2>
            ...
            {/* Items */}
            <div className="flex-1 overflow-y-auto p-2.5 sm:p-4 space-y-2 sm:space-y-4">
              {items.length === 0 ? (
                ...
                items.map((item, index) => (
                  <motion.div
                    ...
                    layout
                    className="flex gap-2.5 sm:gap-4 p-1.5 sm:p-0"
                  >
                    <Link
                      to={`/product/${item.product.id}`}
                      onClick={() => setIsCartOpen(false)}
                      className="w-14 h-18 sm:w-20 sm:h-24 rounded-md overflow-hidden bg-secondary flex-shrink-0"
                    >
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="p-3 sm:p-8 border-t border-border/50 bg-background/60 backdrop-blur-xl relative pb-6 sm:pb-8 mb-2 sm:mb-0">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xl sm:text-3xl font-display font-bold text-foreground">₹{totalPrice.toLocaleString('en-IN')}</span>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="group w-full py-3 sm:py-5 bg-foreground text-background font-bold text-[10px] sm:text-sm uppercase tracking-[0.2em] rounded-xl sm:rounded-2xl hover:opacity-90 transition-all flex items-center justify-center gap-2 overflow-hidden relative"
                  >
                  ...
                  <button
                    ...
                    className="w-full py-3 sm:py-5 bg-[#25D366] text-white flex items-center justify-center gap-2 font-bold text-[10px] sm:text-sm uppercase tracking-[0.15em] rounded-xl sm:rounded-2xl hover:opacity-95 transition-all shadow-lg shadow-[#25D366]/20"
                  >
